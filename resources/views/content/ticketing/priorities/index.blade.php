...
@section('title', 'Priorities Lists')
...
{{--@can('priority_create')--}}
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("priorities.create") }}">
                {{ trans('global.add') }} {{ trans('cruds.priority.title_singular') }}
            </a>
            <a class="btn btn-dark justify-content-end pe-5 " href="{{ url()->previous() }}">
              {{ trans('global.back') }}
            </a>
                ...
                <tbody>
                    @foreach($priorities as $key => $priority)
                        <tr data-entry-id="{{ $priority->id }}">
                            ...
                            <td>
{{--                                @can('priority_show')--}}
                                    <a class="btn btn-xs btn-primary" href="{{ route('priorities.show', $priority->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
{{--                                @endcan--}}

{{--                                @can('priority_edit')--}}
                                    <a class="btn btn-xs btn-info" href="{{ route('priorities.edit', $priority->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
{{--                                @endcan--}}

{{--                                @can('priority_delete')--}}
                                    <form action="{{ route('priorities.destroy', $priority->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('priority_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('priorities.massDestroy') }}",
